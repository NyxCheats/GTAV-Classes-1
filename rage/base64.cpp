...
namespace rage {
	...
	static const char BASE64_ALPHABET [ 65 ] = {"ABCDEFGHIJKLMNOPQRSTUVWXYZ"
	                                            "abcdefghijklmnopqrstuvwxyz"
	                                            "0123456789"
	                                            "+/"};

	unsigned datBase64::GetMaxEncodedSize(const unsigned size) {
		//Add 1 for the terminating null.
		return (((size + 2) / 3) * 4) + 1;
	}

	unsigned datBase64::GetMaxDecodedSize(const char* str) {
	...
	bool datBase64::Encode(const u8* src, const unsigned srcLen, char* dst, const unsigned dstMaxSize, unsigned* charsUsed) {
	...
	bool datBase64::Decode(const char* src, const unsigned dstMaxLen, u8* dst, unsigned* dstLen) {
		bool success = false;
...
#if __ASSERT
	static int TestMe() {
		const char* inputStr [] = {"MTIzNDU2Nzg=", "MTIzNDU2Nzg5", "MTIzNDU2Nzg5MA==", "YWJjZGVmZ2hpamtsbW5vcHFyc3R1dnd4eXo="};
		const char* outputStr [] = {"12345678", "123456789", "1234567890", "abcdefghijklmnopqrstuvwxyz"};
	...
	unsigned datBase64::Decode(DecodeContext* ctx, const char* src, const int srcOffset, const int srcLen, const unsigned dstMaxLen, u8* dst, unsigned* dstLen) {
		int dstIndex = *dstLen;
	...
	int datBase64::CharToValue(const char code) {
		int val;
	...
	bool datBase64::IsValidBase64Char(const char c) {
	...
	bool datBase64::IsValidBase64String(const char* src, const unsigned srcLen) {
