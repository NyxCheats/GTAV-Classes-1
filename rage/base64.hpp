...
#define DAT_BASE64_MAX_ENCODED_SIZE(size) \
	...
	public:
		...
		static bool Encode(const u8* src, const unsigned srcLen, char* dst, const unsigned dstMaxSize, unsigned* charsUsed);

		//PURPOSE
		//  Decodes a base-64 string into binary data.
		//NOTE
		//  This will ignore pad characters.
		static bool Decode(const char* src, const unsigned dstMaxLen, u8* dst, unsigned* dstLen);
		...
		//      }
		//  }
		static unsigned Decode(DecodeContext* ctx, const char* src, const int srcOffset, const int srcLen, const unsigned dstMaxLen, u8* dst, unsigned* dstLen);
