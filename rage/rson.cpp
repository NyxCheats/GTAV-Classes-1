...
#define strnicmp _strnicmp
	...
	#define RSON_SCANF(input, length, format, ...) \
		(Rson::sm_AllowSnscanf ? _snscanf(input, length, format, __VA_ARGS__) : sscanf(input, format, __VA_ARGS__))
#else
    // the length is ignored for platforms that don't have _snscanf - note: not all implementations call strlen()
	#define RSON_SCANF(input, length, format, ...) \
		sscanf(input, format, __VA_ARGS__)
#endif
...
namespace rage {
	...
	bool Rson::IsSpace(const char c) {
		return (c >= 0x09 && c <= 0x0D) || (0x20 == c);
	}

	void Rson::SetAllowSnscanf(const bool snscanf) {
	...
	void RsonWriter::Init(char* buf, const unsigned lenofBuf, const RsonFormat format) {
		m_Buf      = buf;
		m_LenofBuf = lenofBuf;
		m_Format   = format;
		this->Reset();
	...
	void RsonWriter::InitNull(const RsonFormat format) {
		this->Init(NULL, 0, format);
		m_NullBuffer = true;
	}

	void RsonWriter::Reset() {
		m_State  = 0;
		m_Depth  = 0;
		m_Cursor = 0;
	...
	void RsonWriter::ResetToContinue() {
		//Preserve some vars.
	...
	bool RsonWriter::Begin(const char* name, int* cursor) {
	...
	bool RsonWriter::BeginArray(const char* name, int* cursor) {
		if (AssertVerify(m_Depth + 1 <= 32)) {
			if (cursor) {
				*cursor = m_Cursor;
			}
	...
	int RsonWriter::GetCursor() const {
		return m_Cursor;
	}

	int RsonWriter::GetDepth() const {
		return m_Depth;
	}

	void RsonWriter::Undo(const int cursor) {
	...
	bool RsonWriter::WriteInt(const char* name, const int value) {
		return this->Write("%d", name, value);
	}
	bool RsonWriter::WriteInt64(const char* name, const s64 value) {
		return this->Write("%" I64FMT "d", name, value);
	}

	bool RsonWriter::WriteUns(const char* name, const unsigned value) {
		return this->Write("%u", name, value);
	}

	bool RsonWriter::WriteUns64(const char* name, const u64 value) {
	...
	bool RsonWriter::WriteFloat(const char* name, const float value, const char* format) {
		return this->Write(format, name, value);
	}

	bool RsonWriter::WriteHex(const char* name, const unsigned value) {
		if (RSON_FORMAT_JSON == m_Format) {
			char tmp [ 64 ];
	...
	bool RsonWriter::WriteHex64(const char* name, const u64 value) {
		if (RSON_FORMAT_JSON == m_Format) {
			char tmp [ 64 ];
	...
	bool RsonWriter::WriteDouble(const char* name, const double value) {
		return this->Write(DOUBLE_WRITE_FORMAT, name, value);
	}

	bool RsonWriter::WriteBinary(const char* name, const void* value, const unsigned len) {
		if (len == 0) {
		...
		if (!m_NullBuffer) {
			success = datBase64::Encode((const u8*)value, len, &m_Buf [ m_Cursor ], this->Available(), &numConsumed);
		} else {
	...
	bool RsonWriter::WriteBool(const char* name, const bool value) {
		return this->Write("%s", name, value ? "true" : "false");
	}

	bool RsonWriter::WriteString(const char* name, const char* value) {
		// we can have an array of strings, and the
		// strings in the array don't have names
		bool success = (AssertVerify(name || InArray())) && AssertVerify(value) && this->Write("%s", name, "");
	...
	bool RsonWriter::WriteRawString(const char* name, const char* value) {
		return this->Write("%s", name, value);
	}

	template<>
	bool RsonWriter::WriteValue(const char* name, const int value) {
		return this->WriteInt(name, value);
	}

	template<>
	bool RsonWriter::WriteValue(const char* name, const s64 value) {
		return this->WriteInt64(name, value);
	}

	template<>
	bool RsonWriter::WriteValue(const char* name, const unsigned value) {
		return this->WriteUns(name, value);
	}

	template<>
	bool RsonWriter::WriteValue(const char* name, const float value) {
		return this->WriteFloat(name, value);
	}

	template<>
	bool RsonWriter::WriteValue(const char* name, const u64 value) {
		return this->WriteUns64(name, value);
	}

	template<>
	bool RsonWriter::WriteValue(const char* name, const bool value) {
		return this->WriteBool(name, value);
	}

	template<>
	bool RsonWriter::WriteValue(const char* name, char* value) {
		return this->WriteString(name, value);
	}

	template<>
	bool RsonWriter::WriteValue(const char* name, const char* value) {
		return this->WriteString(name, value);
	}

	template<>
	bool RsonWriter::WriteValue(const char* name, const double value) {
	...
	bool RsonWriter::HasError() const {
		return m_Error;
	}

	const char* RsonWriter::ToString() const {
		return m_Buf;
	}

	unsigned RsonWriter::Length() const {
		return m_Cursor;
	}

	//private:

	bool RsonWriter::NeedComma(const char c) const {
		if (RSON_FORMAT_JSON == m_Format) {
			return ('{' != c && '[' != c && ',' != c);
		} else {
			return ('{' != c && ',' != c);
	...
	bool RsonWriter::NeedComma() const {
		return m_NeedComma;
	}

	const char* RsonWriter::Comma() const {
		return this->NeedComma() ? "," : "";
	}

	int RsonWriter::Available() const {
		int avail;
		if (!m_NullBuffer) {
			avail = m_LenofBuf - m_Cursor - 1; //minus one for the terminating nul
		} else {
			avail = 0x7FFFFFFF;
		}
	...
	bool RsonWriter::Write(const char* buf) {
		if (!m_Error) {
	...
	bool RsonWriter::Write(const char c) {
		if (!m_Error) {
			if (this->Available() >= 1) {
				if (!m_NullBuffer) {
					m_Buf [ m_Cursor ]     = c;
	...
	bool RsonWriter::InObject() const {
		return (m_Depth > 0) && (1 == (m_State & 0x01));
	}

	bool RsonWriter::InArray() const {
	...
	static void TestLuaWriter() {
		...
		Assert(0 == strcmp(rw.ToString(), "{int=123,uns=4294906416,float=1.33446,hex=0xABCDEF,hex64=0x123456789ABCDEF,foobar=Zm9vYmFy,bool=true,string=fuckoff,string2=\"fuck off\",string3=\"\\\"fuck off\\\"\",ints={1,2,3,4},uns={1,2,3,4},floats={1.50000,1.25000,1.12500,1.06250}}"));
	...
	RsonReader::RsonReader() :
	    m_Buf(NULL), m_LenofBuf(0), m_NameOffset(-1), m_NameLen(-1), m_ValOffset(-1), m_ValLen(-1) {
	}
	...
	bool RsonReader::Init(const char* buf, const unsigned lenofBuf) {
		return Init(buf, 0, lenofBuf);
	}

	bool RsonReader::Init(const char* const buf, const int offset, const unsigned lenofBuf) {
		this->Clear();
				...
				/*else if(buf[i] == '\"')
                {
					...
					if (i > nameOff) {
						...
						if (i >= 0 && ('=' == buf [ i ] || ':' == buf [ i ])) {
							...
							if (Verifyf((unsigned)nameOff < lenofBuf && nameLen > 0, "Invalid item name at position:%d", nameOff - 1)) {
								++i;
	...
	bool RsonReader::HasData() const {
		return (NULL != m_Buf);
	}

	bool RsonReader::HasMember(const char* name) const {
		RsonReader tmp;
		return GetMember(name, &tmp);
	}

	bool RsonReader::GetFirstMember(RsonReader* rr) const {
		bool success = false;
		if (m_Buf && ('{' == m_Buf [ m_ValOffset ] || '[' == m_Buf [ m_ValOffset ]) && m_ValLen > 0) {
	...
	bool RsonReader::GetNextSibling(RsonReader* rr) {
		bool success = false;
		if (m_Buf) {
	...
	bool RsonReader::GetMember(const char* name, RsonReader* rr) const {
		bool success   = false;
	...
	bool RsonReader::HasDuplicateEntries(const char* name) const {
		// running tally
	...
	bool RsonReader::GetName(char* name, const int maxChars) const {
		bool success = false;

		if (m_Buf && m_NameLen > 0 && Verifyf(maxChars > m_NameLen, "Insufficient buffer length for name")) {
	...
	bool RsonReader::GetValue(char* val, const int maxChars) const {
		bool success = false;

		const unsigned myValLen = this->GetValueLength();

		if (m_Buf && Verifyf((unsigned)maxChars > myValLen, "Insufficient buffer length for value")) {
	...
	bool RsonReader::GetValue(const char* name, char* val, const int maxChars) const {
		RsonReader rr;
		return this->GetMember(name, &rr) && rr.GetValue(val, maxChars);
	}

	int RsonReader::GetValueLength() const {
		return m_Buf ? ('\"' == m_Buf [ m_ValOffset ] ? m_ValLen - 2 : m_ValLen) : 0;
	}

	int RsonReader::GetValueLength(const char* name) const {
		RsonReader rr;
		return this->GetMember(name, &rr) ? rr.GetValueLength() : -1;
	}

	int RsonReader::GetBinaryValueLength() const {
		int len, offset;
	...
	bool RsonReader::AsUns8(u8& val) const {
		bool success  = false;
		const char* p = this->Value();
		if (p) {
			const int valLen = this->GetValueLength();
	...
	bool RsonReader::AsUns16(u16& val) const {
		bool success  = false;
		const char* p = this->Value();
		if (p) {
			const int valLen = this->GetValueLength();
	...
	bool RsonReader::AsUns64(u64& val) const {
		bool success  = false;
		const char* p = this->Value();
		if (p) {
			const int valLen = this->GetValueLength();
	...
	bool RsonReader::AsFloat(float& val) const {
		const char* p = this->Value();
		return p && RSON_SCANF(p, this->GetValueLength(), FLOAT_READ_FORMAT, &val) > 0;
	}

	bool RsonReader::AsDouble(double& val) const {
		const char* p = this->Value();
		return p && RSON_SCANF(p, this->GetValueLength(), DOUBLE_READ_FORMAT, &val) > 0;
	}

	bool RsonReader::AsBinary(void* val, const int maxLen, int* len) const {
		bool success = false;
	...
	bool RsonReader::AsBool(bool& val) const {
		bool success  = false;
		const char* p = this->Value();
		if (p) {
			const unsigned valLen = this->GetValueLength();
			if (4 == valLen) {
				success = ('t' == p [ 0 ] || 'T' == p [ 0 ]) && ('r' == p [ 1 ] || 'R' == p [ 1 ]) && ('u' == p [ 2 ] || 'U' == p [ 2 ]) && ('e' == p [ 3 ] || 'E' == p [ 3 ]);

				if (success) {
					val = true;
				}
			} else if (5 == valLen) {
				success = ('f' == p [ 0 ] || 'F' == p [ 0 ]) && ('a' == p [ 1 ] || 'A' == p [ 1 ]) && ('l' == p [ 2 ] || 'L' == p [ 2 ]) && ('s' == p [ 3 ] || 'S' == p [ 3 ]) && ('e' == p [ 4 ] || 'E' == p [ 4 ]);
	...
	bool RsonReader::AsString(char* val, const int bufLen) const {
		bool success = false;
	...
	bool RsonReader::AsRawString(char* val, const int bufLen) const {
		bool success = false;
	...
	bool RsonReader::AsReader(RsonReader& rr) const {
		bool success = false;
		if (m_Buf) {
	...
	template<>
	bool RsonReader::AsValue(int& value) const {
		return this->AsInt(value);
	}

	template<>
	bool RsonReader::AsValue(s16& value) const {
		return this->AsInt16(value);
	}

	template<>
	bool RsonReader::AsValue(s64& value) const {
		return this->AsInt64(value);
	}

	template<>
	bool RsonReader::AsValue(unsigned& value) const {
		return this->AsUns(value);
	}

	template<>
	bool RsonReader::AsValue(float& value) const {
		return this->AsFloat(value);
	}

	template<>
	bool RsonReader::AsValue(u8& value) const {
		return this->AsUns8(value);
	}

	template<>
	bool RsonReader::AsValue(u16& value) const {
		return this->AsUns16(value);
	}

	template<>
	bool RsonReader::AsValue(u64& value) const {
		return this->AsUns64(value);
	}

	template<>
	bool RsonReader::AsValue(bool& value) const {
		return this->AsBool(value);
	}

	template<>
	bool RsonReader::AsValue(double& value) const {
		return this->AsDouble(value);
	}

	template<>
	bool RsonReader::AsValue(RsonReader& value) const {
		return this->AsReader(value);
	}

	static const char* NULL_VALUE = "null";

	bool RsonReader::IsNull() const {
		const char* p = this->Value();
		return p && !strnicmp(p, NULL_VALUE, strlen(NULL_VALUE));
	}

	bool RsonReader::ReadInt(const char* name, int& val) const {
		RsonReader rr;
		return this->GetMember(name, &rr) && rr.AsInt(val);
	}

	bool RsonReader::ReadInt16(const char* name, s16& val) const {
		RsonReader rr;
		return this->GetMember(name, &rr) && rr.AsInt16(val);
	}

	bool RsonReader::ReadInt64(const char* name, s64& val) const {
		RsonReader rr;
		return this->GetMember(name, &rr) && rr.AsInt64(val);
	}

	bool RsonReader::ReadUns(const char* name, unsigned& val) const {
		RsonReader rr;
		return this->GetMember(name, &rr) && rr.AsUns(val);
	}

	bool RsonReader::ReadUns16(const char* name, u16& val) const {
		RsonReader rr;
		return this->GetMember(name, &rr) && rr.AsUns16(val);
	}

	bool RsonReader::ReadUns64(const char* name, u64& val) const {
		RsonReader rr;
		return this->GetMember(name, &rr) && rr.AsUns64(val);
	}

	bool RsonReader::ReadFloat(const char* name, float& val) const {
		RsonReader rr;
		return this->GetMember(name, &rr) && rr.AsFloat(val);
	}

	bool RsonReader::ReadDouble(const char* name, double& val) const {
		RsonReader rr;
		return this->GetMember(name, &rr) && rr.AsDouble(val);
	}

	bool RsonReader::ReadBinary(const char* name, void* val, const int maxLen, int* len) const {
		RsonReader rr;
		return this->GetMember(name, &rr) && rr.AsBinary(val, maxLen, len);
	}

	bool RsonReader::ReadBool(const char* name, bool& val) const {
		RsonReader rr;
		return this->GetMember(name, &rr) && rr.AsBool(val);
	}

	bool RsonReader::ReadString(const char* name, char* val, const int bufLen) const {
		RsonReader rr;
		return this->GetMember(name, &rr) && rr.AsString(val, bufLen);
	}

	bool RsonReader::ReadRawString(const char* name, char* val, const int bufLen) const {
		RsonReader rr;
		return this->GetMember(name, &rr) && rr.AsRawString(val, bufLen);
	}

	bool RsonReader::ReadReader(const char* name, RsonReader& val) const {
		RsonReader rr;
	...
	void RsonReader::GetB64OffsetAndLength(int* offset, int* len) const {
	...
	int RsonReader::SkipWs(const char* buf, const int offset, const unsigned lenofBuf) {
		Assert(offset >= 0);
	...
	int RsonReader::EndOfToken(const char* buf, const int offset, const unsigned lenofBuf) {
		Assert(offset >= 0);
		Assert((unsigned)offset < lenofBuf);
	...
	int RsonReader::EndOfString(const char* buf, const int offset, const unsigned lenofBuf) {
		Assert(offset >= 0);
		Assert((unsigned)offset < lenofBuf);
	...
	int RsonReader::EndOfObject(const char* buf, const int offset, const unsigned lenofBuf) {
		Assert(offset >= 0);
		Assert((unsigned)offset < lenofBuf);
	...
	bool RsonReader::CheckName(const char* name, const bool ignoreNull) const {
		if (!m_Buf || m_NameOffset < 0 || m_NameLen <= 0)
			return false;
		if (strncmp(name, &m_Buf [ m_NameOffset ], m_NameLen))
			return false;
		if (ignoreNull && IsNull())
			return false;

		return true;
	}

	void RsonReader::GetRawValueString(char* buf, int bufSize) const {
	...
	const char* RsonReader::Value() const {
		return (Verifyf(m_Buf && m_ValOffset >= 0 && m_ValLen > 0, "")) ? ('\"' == m_Buf [ m_ValOffset ] ? &m_Buf [ m_ValOffset + 1 ] : &m_Buf [ m_ValOffset ]) : "";
	}
...
#undef D
	...
	bool RsonReader::ValidateJson(const char* buf, int bufSize) {
		JsonValidator v(buf, bufSize);
		return v.ValidateDocument();
	}

	bool RsonReader::ValidateJsonObject(const char* buf, int bufSize, bool bSkipBraces) {
		JsonValidator v(buf, bufSize);
	...
	const char* RsonReader::Name() const {
		return (m_Buf && Verifyf(m_NameOffset >= 0, "")) ? &m_Buf [ m_NameOffset ] : "";
	}

	const char* RsonReader::GetRawMemberValue(const char* name, int& outStrLen) const {
		RsonReader tmp;
	...
	BrsonWriter::BrsonWriter() :
	    m_OrigBuf(NULL), m_Buf(NULL), m_Cursor(0), m_BufLength(0) {
	}
	...
	void BrsonWriter::Init(u8* buf, const unsigned len) {
		Clear();
	...
	void BrsonWriter::Clear() {
		m_OrigBuf = m_Buf = NULL;
		m_Cursor = m_BufLength = 0;
	}

	bool BrsonWriter::Begin(const char* name, unsigned* cursor) {
	...
	bool BrsonWriter::WriteString(const char* name, const char* value) {
		return WriteString(name) && WriteString(value);
	}

	bool BrsonWriter::WriteInt(const char* name, const s64 value) {
		if (WriteString(name)) {
	...
	bool BrsonWriter::WriteVarInt(const u64 val) {
		u64 u = val;
		do {
	...
	bool BrsonWriter::WriteString(const char* str) {
	...
	BrsonReader::BrsonReader() :
	    m_ParentBuf(NULL), m_ParentLen(0), m_Buf(NULL), m_Length(0) {
	}

	bool BrsonReader::Init(const u8* buf, const size_t len) {
		if (!buf || !len) {
			return false;
		}
	...
	bool BrsonReader::GetFirstMember(BrsonReader* rr) const {
		if (BRSONTYPE_OBJECT == m_Buf [ 0 ] || BRSONTYPE_ARRAY == m_Buf [ 0 ]) {
			size_t offset;
			u64 varInt;
	...
	bool BrsonReader::GetNextSibling(BrsonReader* rr) const {
		if (m_ParentBuf) {
	...
	bool BrsonReader::GetName(char* name, const unsigned maxChars) const {
		if (!m_Buf || !m_Length) {
			return false;
		}
	...
	size_t BrsonReader::ReadVarInt(const u8* buf, const size_t len, u64* varInt) {
		*varInt     = 0;
		int shift   = 0;
		const u8* p = buf;
		do {
