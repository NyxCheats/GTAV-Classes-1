...
#define RGSC_AUMID_DEBUG L"RockstarGames.SocialClub.UI.Debug"
	...
	inline bool operator==(RGSC_REFIID src1, RGSC_REFIID src2) {
		return ((src1.Data1 == src2.Data1) && (src1.Data2 == src2.Data2) && (src1.Data3 == src2.Data3)
		        && (*(__int64*)src1.Data4 == *(__int64*)src2.Data4)) ?
		    true :
		    false;
	}
...
#define DECLARE_WINDOWHANDLE(name) \
	struct name##__ {              \
		int unused;                \
	};                             \
...
#define RGSC_CALL __stdcall
	...
	template<typename T>
	int RgscStringLength(const T* s) {
		if (s) {
			const T* e = s;
	...
	template<typename T>
	T* RgscStringDuplicate(const T* str) {
		if (!str)
			return 0;
	...
	template<typename T>
	inline void RgscStringFree(T* str) {
		delete [] (T*)str;
	}
