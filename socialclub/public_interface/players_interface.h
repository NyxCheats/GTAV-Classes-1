...
#define IID_IPlayerListLatestVersion IID_IPlayerListV1
	...
	*/
		virtual RGSC_HRESULT RGSC_CALL CreatePlayerEnumerator(RGSC_REFIID riid, const RockstarId rockstarId, const PlayerListType playerListType, const u32 firstPlayerIndex, const u32 maxPlayersToRead, u32* numBytesRequired, void** handle) = 0;
	  ...
	  \return
		...
		ERROR_NOT_LOGGED_ON - if the local player is not online.
	*/
		virtual RGSC_HRESULT RGSC_CALL EnumeratePlayers(void* handle, void* buffer, const u32 bufferSize, IPlayerList** players, IAsyncStatus* status) = 0;
