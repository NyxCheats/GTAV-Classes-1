	...
	public:
		...
		enum DetailFlags {
			...
			ACHIEVEMENT_DETAILS_ALL = ACHIEVEMENT_DETAILS_STRINGS | //!< retrieves all data
			    ACHIEVEMENT_DETAILS_IMAGE | ACHIEVEMENT_DETAILS_IS_AWARDED | ACHIEVEMENT_DETAILS_TIMESTAMP,
		};
...
#define IID_IAchievementListLatestVersion IID_IAchievementListV1
	...
	*/
		virtual RGSC_HRESULT RGSC_CALL CreateAchievementEnumerator(RGSC_REFIID riid, const ITitleId* titleId, const RockstarId profileId, const IAchievement::DetailFlags detailFlags, const AchievementId firstAchievementId, const u32 maxAchievementsToRead, u32* numBytesRequired, void** handle) = 0;
	  ...
	  \return
		...
		ERROR_NOT_LOGGED_ON - if the local player is not signed in.
	*/
		virtual RGSC_HRESULT RGSC_CALL EnumerateAchievements(void* handle, void* buffer, const u32 bufferSize, IAchievementList** achievements) = 0;
	...
	*/
	...
	public:
	  ...
	  \return
		...
		ERROR_NOT_LOGGED_ON - if the local player is not signed in.
	*/
		virtual RGSC_HRESULT RGSC_CALL EnumerateAchievements(void* handle, void* buffer, const u32 bufferSize, IAchievementList** achievements, IAsyncStatus* status) = 0;
	};
